...
func (su *Supervisor) load() ([]byte, []byte, error) {
	su.mu.Lock()
	defer su.mu.Unlock()

	oob := []byte{}
	if su.fd > 0 {
		oob = syscall.UnixRights(su.fd)
	}
...
func recv(uf *os.File) ([]byte, []byte, error) {
	data := make([]byte, 0)
	oob := make([]byte, 0)
	...
	for {
		...
		n, oobn, _, _, err := unix.Recvmsg(int(uf.Fd()), dataBuf, oobBuf, 0)
		if err != nil {
			if err == unix.EAGAIN || err == unix.EINTR {
				continue
			}
			return nil, nil, errors.Wrap(err, "receive message")
		}
...
func (su *Supervisor) waitStatesTimeout(to time.Duration) (func() error, error) {
	...
	receiver := func() error {
		...
		data, oob, err := recv(uf)
		if err != nil {
			return err
		}
		log.L.Infof("Supervisor %s receives states. data %d oob %d", su.id, len(data), len(oob))
...
func (su *Supervisor) SendStatesTimeout(to time.Duration) error {
	...
	sender := func() error {
		...
		log.L.Infof("Supervisor %s sends states. data %d oob %d", su.id, len(data), len(oob))
