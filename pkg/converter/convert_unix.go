	...
	err := seekFileByTarHeader(ra, EntryTOC, func(tocEntryDataReader io.Reader, tarHeaderReader io.Reader) error {
		...
		for i := 0; i < count; i++ {
			...
			if entry.GetName() == targetName {
				...
				blobMetaHeaderSize := int64(4096)

				var sr *io.SectionReader
				if targetName == EntryBlobMeta {
					if entry.GetCompressedSize() < uint64(blobMetaHeaderSize) {
						return fmt.Errorf("invalid %s header size", EntryBlobMeta)
					}
					sr = io.NewSectionReader(ra, compressedOffset, compressedSize-blobMetaHeaderSize)
				} else {
				...
				if targetName == EntryBlobMeta {
					uncompressedBlobMetaData, err := io.ReadAll(rd)
					if err != nil {
						return errors.Wrap(err, "decompress blob meta data")
					}
					uncompressedBlobMetaDataSize := int64(len(uncompressedBlobMetaData))
					paddingSize := roundUp4K(uncompressedBlobMetaDataSize) - uncompressedBlobMetaDataSize
					paddingReader := bytes.NewBuffer(newPadding(paddingSize))
					blobMetaHeaderReader := io.NewSectionReader(ra, compressedOffset+compressedSize-blobMetaHeaderSize, blobMetaHeaderSize)
					rd = io.MultiReader(bytes.NewReader(uncompressedBlobMetaData), paddingReader, blobMetaHeaderReader)
				}
