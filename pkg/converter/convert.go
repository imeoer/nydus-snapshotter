...
func Convert(ctx context.Context, dest io.Writer, opt ConvertOption) (io.WriteCloser, func(context.Context) error, error) {
	workDir, err := ioutil.TempDir(os.Getenv(envNydusWorkdir), "nydus-converter-")
	if err != nil {
		return nil, nil, errors.Wrap(err, "create work directory")
	}
	defer func() {
		if err != nil {
	...
	if err := os.MkdirAll(sourceDir, 0755); err != nil {
		return nil, nil, errors.Wrap(err, "create source directory")
	}
